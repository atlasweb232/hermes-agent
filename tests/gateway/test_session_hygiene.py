...
@pytest.mark.asyncio
async def test_session_hygiene_hard_limit_ignores_offloaded_tool_refs(
    monkeypatch, tmp_path
):
    """Offloaded tool refs should not force compression by message count.

    Workload execution can create many tool/progress entries. Once the
    context-admission gate has replaced those entries with event refs, they no
    longer consume meaningful prompt context, so the hard message-count safety
    valve should count only context-bearing messages.
    """
    fake_dotenv = types.ModuleType("dotenv")
    fake_dotenv.load_dotenv = lambda *args, **kwargs: None
    monkeypatch.setitem(sys.modules, "dotenv", fake_dotenv)
    ...
    history = _make_history(4, content_size=20)
    history.extend(
        {
            "role": "tool",
            "tool_name": "terminal",
            "_context_offloaded": True,
            "content": (
                "<context-offloaded-transcript-tool-result>\n"
                "event_ref: event://worker/example\n"
