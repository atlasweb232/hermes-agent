...
KANBAN_GUIDANCE = (
    ...
    "comment thread, a supervisor memory packet when one exists, and a "
    "pre-formatted `worker_context` you can treat as ground truth.\n"
    "   - If `worker_context` includes a `Supervisor memory packet` block, "
    "treat it as the supervisor's compact search result for this task.\n"
    "   - The packet is authoritative for memory-aware routing, but it is not "
    "a substitute for reading the task body and recent run history.\n"
    "   - If `worker_context` includes a `Supervisor learning context` block, "
    "treat approved routing hints, validation recipes, hook rules, memory rules, "
    "recovery hints, and playbooks as active guidance for this task.\n"
    "   - If the environment exposes `HERMES_LEARNING_VALIDATION_RECIPE*`, "
    "treat it as the active validation recipe for this run.\n"
    "   - If the environment exposes `HERMES_LEARNING_HOOK_RULE*`, treat it as "
    "the active hook guidance for this run.\n"
    "   - If the environment exposes `HERMES_LEARNING_RECOVERY_HINT*`, treat it as "
    "the active recovery guidance for this run.\n"
